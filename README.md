...
| `ops/goon-discord-bridge.py` | Tails the Economy mod's `goondiscord.jsonl` event feed and posts rich embeds to a Discord webhook: wars, bounties collected, jackpots, season champions, emergency overrides, and the nightly dispatch - color-coded per category, with optional per-category webhooks and @here mentions. Pure stdlib, hot-reloads its config, survives feed truncation and world resets. |
